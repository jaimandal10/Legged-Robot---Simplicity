...
import simplicity_env


if __name__ == '__main__':
    
    rospy.init_node('simplicity_gym', anonymous=True, log_level=rospy.INFO)

    # Create the Gym environment
    env = gym.make('simplicity-v0')
    rospy.logdebug ( "Gym environment done")
    reward_pub = rospy.Publisher('/simplicity/reward', Float64, queue_size=1)
    episode_reward_pub = rospy.Publisher('/simplicity/episode_reward', Float64, queue_size=1)

    # Set the logging system
    rospack = rospkg.RosPack()
    pkg_path = rospack.get_path('simplicity_description')
